...
The model is the same as the one FSDP however, the internal loss computation has been stripped 
out completely.
"""
import math
import torch
...
class GPT(nn.Module):
    ...
    def forward(self, x):
        if self.tok_emb is not None:
            # stage 0: x is raw token ids, shape (B, T)
            B, T = x.shape
            pos = torch.arange(0, T, device=x.device).unsqueeze(0)
            x = self.drop(self.tok_emb(x) + self.pos_emb(pos))
            # else: stage 1, x is already hidden states handed off from stage 0, shape (B, T, C)
